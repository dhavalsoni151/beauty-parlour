...
///
/// Used by [FirestoreBackupService] (see `firestore_backup_service.dart`) when
/// importing an old flat-schema JSON backup (from before the app used the
/// normalized Category->ServiceType->Service hierarchy) into Firestore.
///
/// This file contains NO Flutter / Firebase imports on purpose so the mapping
/// heuristics and the [MigrationReport] stay easy to unit-test with plain
/// `dart test` (no Flutter bindings required).
library;
...
/// Aggregated result of a migration/import run (legacy flat JSON, normalized
/// SQLite-shaped JSON, or a native Firestore export).
...
class MigrationReport {
  ...
  // Financial totals computed from the ORIGINAL source JSON.
  double sourceVisitsTotal = 0;
  double sourcePaymentsTotal = 0;
  double sourceExpensesTotal = 0;

  // Financial totals recomputed from the MIGRATED/restored data.
  ...
  String buildSummary() {
    ...
    b.writeln('Import complete.');
    b.writeln('');
    b.writeln('Records restored:');
    ...
    b.writeln('  Reminders: $reminders');
    b.writeln('');
    b.writeln('Financial reconciliation (source -> restored):');
